...
class Parser:
    ...
    def consume(self, expected_type): # Consumes the next token and checks its type
        if self.lookahead() and self.lookahead()[0] == expected_type: # Check that we haven't reached the end and that the next token is the expected type
            self.position += 1
            return self.tokens[self.position - 1]
        else:
            raise RuntimeError(f'Expected token type {expected_type}, got {self.lookahead()}')
    ...
    def term(self):
        node = self.factor()
        while self.lookahead() and self.lookahead()[0] == 'OPERATOR' and self.lookahead()[1] in ('*', '/'):
